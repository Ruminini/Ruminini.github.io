...
class Particle {
  constructor(x, y) {
    ...
    this.col = `hsla(${Math.random()*50+135}, 100%, ${Math.random()*40+10}%, ${colorConfig.particleOpacity})`;
  }
...
function reset() {
  ...
  imgSize += imgSize/dpr < 300 ? imgSize * .5 : 0;
  if (w > h) {
...
function draw() {
  ...
  drawParticles();
  let t3 = Date.now();
  calcTimes.push(t2 - t1);
  drawTimes.push(t3 - t2);
  cleaner++;
  if(cleaner > 30) {
    drawBackground(0.02);
    cleaner = 0;
...
function printFps() {
  ...
  time = Date.now();
  if (times.length > 60) {
    console.log(Math.round(1000 / (times.reduce((a, b) => a + b) / times.length)),
    Math.round(times.reduce((a, b) => a + b) / times.length),
    Math.round(calcTimes.reduce((a, b) => a + b) / calcTimes.length),
    Math.round(drawTimes.reduce((a, b) => a + b) / drawTimes.length));
    times = [];
    calcTimes = [];
    drawTimes = [];
  }
  ...
  for(let x = 0; x < columns; x++) {
    for(let y = 0; y < rows; y++) {
      ...
      if (color) {
        ...
        x1 = simplex3(x/config.zoom, y/config.zoom, noiseZ) * config.fieldForce / 20;
        y1 = simplex3(x/config.zoom + 40000, y/config.zoom + 40000, noiseZ) * config.fieldForce / 20;
      }
      field[x][y].x = x1;
      field[x][y].y = y1;
    }
