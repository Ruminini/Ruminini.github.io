...
function calculateField() {
	fieldFrame++;
	if (fieldFrame % fieldUpdateInterval !== 0) return;

	// 1. Build a full-resolution logo mask (cheap — just array lookups)
	...
	// 2. Calculate low-res noise field (only for non-logo cells)
	let lowField = new Array(fieldColumns);
	for (let x = 0; x < fieldColumns; x++) {
		lowField[x] = new Array(fieldRows);
		for (let y = 0; y < fieldRows; y++) {
			let nx = (x * fieldScale) / config.zoom;
			let ny = (y * fieldScale) / config.zoom;
			lowField[x][y] = {
				x: (simplex3(nx, ny, noiseZ) * config.fieldForce) / 20,
				y: (simplex3(nx + 40000, ny + 40000, noiseZ) * config.fieldForce) / 20,
			};
		}
	}

	// 3. Fill full-res field: use logo mask at full res, interpolated noise elsewhere
	for (let x = 0; x < columns; x++) {
		for (let y = 0; y < rows; y++) {
			if (logoMask[y * columns + x]) {
				// Logo pixel: random force at full resolution (preserves logo shape)
				field[x][y].x = (Math.random() - 0.5) * config.randomForce;
				field[x][y].y = (Math.random() - 0.5) * config.randomForce;
			} else {
				// Non-logo: bilinear interpolation from low-res noise
				let lx = x / fieldScale;
				let ly = y / fieldScale;
				...
				let top = lerp(lowField[x0][y0], lowField[x1][y0], fx);
				let bottom = lerp(lowField[x0][y1], lowField[x1][y1], fx);

				field[x][y].x = top.x + (bottom.x - top.x) * fy;
				field[x][y].y = top.y + (bottom.y - top.y) * fy;
			}
		}
	}
}
